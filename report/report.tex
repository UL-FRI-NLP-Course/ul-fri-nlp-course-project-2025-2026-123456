...
\textbf{Database attribute embeddings evaluation.} We designed a benchamrk to evaluate the performance of different embedding models for the task of identifying relevant attributes from user queries. We created a dataset of 40 user queries, each manually annotated with the relevant attributes. We then tested several embedding models, from Qwen, BAAI-bge, nomic-ai, GTE, Google's gemma and jinaai. To rank the models we measured Spearman's correlation to measure how well the model's ranking of attributes agrees with the ground truth. We also measured the separability of the embeddings, since we want a large gap between the relevant and irrelevant attributes, and finally we also measured the F1 score, which was also optimized with different thresholds. Results of the benchmark were then ranked models with their optimal thresholds. The best model was \texttt{Qwen3-Embedding-4B}, with \texttt{jina-embeddings-v5-text-small} in the close second place. We however used \texttt{\textbf{Qwen3-Embedding-0.6B}} which ranked 3rd but had the best performance compared to its speed.
