...
which yields the first candidate set. Next we embed not user's latest query but the 
entire conversation history. Since the user's responses might be incomplete, 
using the full dialoge helps capture user's intent. 
This embedding is then used for semantic search in the FAISS vector database, which returns 
relevant parts from vehicle brochures. From that text we extract the second set of cars candidates,
which is again filtered using the previously obtainet constraints. 
...
is not fully automated. We use the same queries as before, but simulate 
multi-turn interaction by providing follow-up responses after each LLM question. 
...
the manually parsed query. We also join user's multiple queries into one and 
send them to the non-conversational and baseline LLM. It is not fully 
fair, but we also compare evaluation of our main pipeline with these two.
